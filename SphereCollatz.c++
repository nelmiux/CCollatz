...
int collatz_eval (int i, int j) {
    ...
    for (int k = start; k <= end; k++) {
        #if defined(meta_cache) && defined(lazy_cache)
            if (k % 1000 == 1 && end - k >= 1000) {
                cl = meta_cache[k/1000];
                k += 1000;
            } else {
                if (lazy_cache[k] != 0) cl = lazy_cache[k];
                else {
                    cl = collatz_length(k);
                    lazy_cache[k] = cl;
                }
